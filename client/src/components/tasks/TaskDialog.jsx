...
const ChangeTaskActions = ({ _id, stage, refetch }) => {
  ...
  // Định nghĩa workflow nghiêm ngặt
  ...
  // Tạo tất cả stage options nhưng chỉ enable những stage có thể chuyển đổi
  const items = Object.keys(stageConfig).map(stageKey => {
    const isAvailable = validTransitions[stage]?.includes(stageKey);
    const isCurrentStage = stage === stageKey;
    
    return {
      label: stageConfig[stageKey].label,
      stage: stageKey,
      icon: stageConfig[stageKey].icon,
      onClick: isAvailable ? () => changeHanlder(stageKey) : null,
        ...
        <CustomTransition>
          <Menu.Items className='absolute p-4 left-0 mt-2 w-40 divide-y divide-gray-100 rounded-md bg-white shadow-lg ring-1 ring-black/5 focus:outline-none'>
            <div className='px-1 py-1 space-y-2'>
              {items.map((el) => (
                <Menu.Item key={el.label} disabled={el.disabled}>
                  {({ active }) => (
                    <button
                      disabled={el.disabled}
                      onClick={el?.onClick}
                      className={clsx(
                        active && !el.disabled ? "bg-gray-200 text-gray-900" : "text-gray-900",
            ...
            <Menu.Items className='absolute p-4 right-0 mt-2 w-56 origin-top-right divide-y divide-gray-100 rounded-md bg-white shadow-lg ring-1 ring-black/5 focus:outline-none'>
              <div className='px-1 py-1 space-y-2'>
                {items.map((el, index) => (
                  <Menu.Item key={el.label}>
                    {({ active }) => (
                      <button
                        disabled={index === 0 ? false : !user.isProjectManager}
                        onClick={el?.onClick}
                        className={`${
              ...
              <div className='px-1 py-1'>
                <Menu.Item>
                  {({ active }) => (
                    <button
                      disabled={!user.isProjectManager}

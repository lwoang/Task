              ...
              <div className='flex items-center gap-2'>
                ...
                <div className='flex items-center'>
                  <div className={clsx(
                    ...
                    <UserInfo user={task.projectManager} />
                  </div>
                </div>
              </div>
            )}
              ...
              {task?.team?.length > 0 &&
                task?.team?.map((m, index) => (
                  <div
                    key={index}
                    className={clsx(
                      "w-6 h-6 rounded-full text-white flex items-center justify-center text-xs -mr-1",
                      BGS[(index + 1) % BGS?.length]
                    )}
                  >
                    <UserInfo user={m} />
                  </div>
          ...
          <div className='py-4 border-t border-gray-200 dark:border-gray-700'>
            ...
            <div className='p-4 space-x-8'>
              <span className='text-sm text-gray-600 dark:text-gray-500'>
                {formatDate(new Date(task?.subTasks[0]?.dueDate))}
              </span>

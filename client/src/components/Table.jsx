...
export default function Table({ tasks, refetch }) {
    ...
    <tr className='border-b border-gray-200 text-gray-600 hover:bg-gray-300/10'>
      ...
      <td className='py-2 flex gap-2 md:gap-4 justify-end'>
        {user?.isProjectManager ? (
          <>
            <Button
              className='text-blue-600 hover:text-blue-500 text-sm md:text-base px-4 py-1.5 rounded-md'
              label='Edit'
              type='button'
              onClick={() => editClickHandler(task)}
            />
            <Button
              className='text-red-700 hover:text-red-500 text-sm md:text-base px-4 py-1.5 rounded-md'
              label='Delete'
              type='button'
